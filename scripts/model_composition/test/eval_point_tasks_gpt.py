...
class OpenAIGPT():
    ...
    def safe_chat_complete(self, messages, content_only=True, **kwargs):
        ...
        response = requests.post(url=url,data=data, auth=HTTPBasicAuth(username="cc",password="Thumt4@2023")).json()
        if content_only:
            response = response['choices'][0]["message"]['content']
...
class OpenAICloseSetClsEvaluator(OpenAIOpenFreeFormClsEvaluator):
    def __init__(self, inputs, output_dir, output_file, model_type="gpt-3.5-turbo-0613", synthetic=False):
        ...
        try:
            ...
            catfile = os.path.join(os.path.dirname(__file__), 'modelnet40_shape_names_modified.txt') # * i.e. pointllm/data/modelnet_config/modelnet40_shape_names_modified.txt
    ...
    def evaluate_result(self, result):
        ...
        model_output = result['model_output']
        if self.synthetic:
            accuracy = 1 if ground_truth_label.lower() in model_output.lower() else 0
            cls_result = ""
            cls_label = ground_truth if ground_truth_label.lower() in model_output.lower() else -1
            reason = ""
            prompt_tokens = 0
            completion_tokens = 0
        else:
...
def start_evaluation(results, output_dir, output_file, eval_type="open-free-form-classification", model_type="gpt-3.5-turbo-0613",
                        parallel=True, num_workers=20, synthetic=False):
    """
    Args:
    ...
    if eval_type == "open-free-form-classification":
        ...
        evaluator = OpenAICloseSetClsEvaluator(results, output_dir, output_file, model_type=model_type, synthetic=synthetic)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_type", type=str, default="gpt-4-0613", choices=["gpt-3.5-turbo-0613", "gpt-3.5-turbo-1106", "gpt-4-0613", "gpt-4-1106-preview", "gpt-4-0125-preview"], help="Type of the model used to evaluate.")
    ...
    if args.synthetic:
        if args.eval_type == 'modelnet-close-set-classification':
            output_file = os.path.basename(args.results_path).replace(".json", f"_evaluated_synth.json")
        else:
            raise NotImplementedError(f"Synthetic only for eval_type modelnet-close-set-classification, but get {args.eval_type}")
    ...
    start_evaluation(results=args.results_path, output_dir=args.output_dir, output_file=output_file, eval_type=args.eval_type, model_type=args.model_type, 
                        parallel=args.parallel, num_workers=args.num_workers, synthetic=args.synthetic)
