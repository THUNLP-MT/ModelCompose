...
def load_and_transform_video(
    ...
    if video_path.endswith('.jpg'):
        # Image data
        from PIL import Image
        video_data = Image.open(video_path).convert('RGB') if isinstance(video_path, str) else video_path
        video_data = transforms.ToTensor()(video_data)
        video_data = video_data.unsqueeze(1)
        video_outputs = transform_image(video_data) # (C, 1, H, W)
