...
class LanguageBindVideoTower(nn.Module):
    def __init__(self, video_tower, args, delay_load=False, cache_dir='./cache_dir'):
        ...
        self.select_layer = args.mm_video_select_layer
        self.select_feature = getattr(args, 'mm_video_select_feature', 'patch')
    ...
    def dummy_inputs(self):
        return torch.zeros(1, self.config.num_channels, self.config.num_frames, self.config.image_size, self.config.image_size, device=self.device, dtype=self.dtype)
