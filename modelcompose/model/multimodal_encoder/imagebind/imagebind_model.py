...
class ImageBindModel(nn.Module):
    def __init__(
        ...
        assert forward_select in [0,1] , "forward_select: get_audio_feature() returns audio_feature(0), modality_value(1)"
        ...
        # [MULTIMODAL LLAMA]
        # self.modal_processor = self.modality_preprocessors[ModalityType.AUDIO]
    ...
    def forward(self, inputs):
        # We only use the audio part
            # print(inputs.shape)
            # inputs = einops.rearrange(inputs, 'b c t h w -> b t c h w')
            ret = self.get_audio_feature(inputs, ModalityType.AUDIO)[1] 
            return ret
        # outputs = {}
    ...
    def load_model(self):
        if (self.ckpt_path is not None) and (os.path.exists(self.ckpt_path)):
            load_dict = torch.load(self.ckpt_path)
            
            from transformers.modeling_utils import _load_state_dict_into_model
            _load_state_dict_into_model(self, load_dict, '')
            ...
            print("Can't load ImageBindModel, since ckpt_path is invalid.")


def imagebind_huge(ckpt_path=None, delay_load=False, pretrained=True, forward_select=1):
    ...
    return model #audio_hidden_size=1024
