...
class CustomDataset(Dataset):
    def __init__(self, questions, image_folder, tokenizer, image_processor, model_config, add_image_token):
    ...
    def __getitem__(self, index):
        ...
        if isinstance(image_file, list):
            images = [Image.open(os.path.join(self.image_folder, fn)).convert('RGB') for fn in image_file]
...
# DataLoader
def create_data_loader(questions, image_folder, tokenizer, image_processor, model_config, batch_size=1, num_workers=4, add_image_token=True):
    assert batch_size == 1, "batch_size must be 1"
    dataset = CustomDataset(questions, image_folder, tokenizer, image_processor, model_config, add_image_token)
...
def eval_model(args):
    ...
    data_loader = create_data_loader(questions, args.image_folder, tokenizer, image_processor, model.config, add_image_token=not args.no_add_image_token)
    ...
    for (input_ids, image_tensor), line in tqdm(zip(data_loader, questions), total=len(questions)):
        ...
        label_idx = input_ids[0].tolist()[::-1].index(29962)
        labels[0, -label_idx:] = input_ids[0, -label_idx:]
        ...
        input_ids[:, [-3, -2, -1]] = input_ids[:, [-2, -3, -1]]
        labels[:, [-3, -2, -1]] = labels[:, [-2, -3, -1]]
        ...
        with torch.inference_mode():
            ...
            all_loss = all_loss + outputs.loss.cpu().item()
            cnt += 1
        # ans_file.flush()
